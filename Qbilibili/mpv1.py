...
class MPV(object):
    ...
    def __init__(self, log_handler=None, **kwargs):
        ...
        _mpv_set_option_string(self.handle, b'vo', b'opengl:pbo:dither=no:alpha=no')
        #  _mpv_set_option_string(self.handle, b'hwdec', b'videotoolbox')
        #  _mpv_set_option_string(self.handle, b'display-fps', b'60')
    ...
    def mpv_set_option(self, a, c):
        #  _mpv_set_option(self.handle, a.encode(), MpvFormat.INT64, ctypes.c_void_p(int(c)))
        getattr(backend, 'mpv_set_option')(self.handle, a.encode(), MpvFormat.INT64, ctypes.c_void_p(c))

    def mpv_set_property_async(self, name, f, data):
        _mpv_set_property_async(self.handle, ctypes.c_longlong(0), ctypes.c_char_p(name.encode()), getattr(MpvFormat, f), ctypes.c_void_p(data))

    def mpv_set_property_string(self, name, value):
        _mpv_set_property_string(self.handle, str(name).encode(), str(name).encode())
